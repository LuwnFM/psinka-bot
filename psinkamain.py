...
# ============================================================================
# 🔧 RAILWAY-ОПТИМИЗАЦИИ (ДОБАВЛЕНО)
# ============================================================================

# Определяем, запущены ли на Railway
IS_RAILWAY = os.getenv('RAILWAY', '').lower() == 'true'

# Безопасные значения для Railway
DEFAULT_MAX_CONCURRENT = 2 if IS_RAILWAY else 10
DEFAULT_REQUESTS_PER_COMBO = 1 if IS_RAILWAY else 2

# Настройка логгирования — вывод в консоль (Railway показывает логи в панели)
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    handlers=[
        logging.StreamHandler(),  # Обязательно для Railway!
        # FileHandler оставляем, но с защитой
...
# ============================================================================
# 🛡️ БЕЗОПАСНАЯ ЗАПИСЬ ФАЙЛОВ (для Railway)
# ============================================================================

def safe_write_file(filepath: str, content: str, max_size_mb: int = 5):
    """
    Безопасная запись файла с проверкой размера и обработкой ошибок.
    На Railway файловая система временная — не критично, если запись не удастся.
    """
    try:
...
# ============================================================================
# ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (ваши оригинальные + небольшие правки)
# ============================================================================

async def make_g4f_request(provider_name: str, model: str, prompt: str,
                           timeout: float = 40.0,
                           system_prompt: str = None) -> Tuple[bool, str, float]:
    """
    Делает запрос к g4f (ТОЛЬКО АСИНХРОННЫЙ).
    Возвращает: (успех, ответ/ошибка, время_сек)
    """
    elapsed = 0.0
    start = time.time()
...
async def get_openrouter_free_models() -> List[str]:
    """
    Динамически получает список бесплатных моделей через OpenRouter API.
    """
...
async def test_openrouter_model(model: str, prompt: str, timeout: float = 35.0):
    """
    Тестирует одну модель OpenRouter.
    """
    openrouter_token = os.getenv('OPENR_TOKEN')
    ...
    try:
        ...
        def make_request():
            ...
            if not is_google_model:
                messages.append({
                    "role": "system",
                    "content": "Отвечай максимально кратко. Если это тест — просто напиши 'ок'."
                })
            ...
            return client.chat.completions.create(
                ...
                extra_headers={
                    ...
                    "X-OpenRouter-Title": "PsIInka Bot",
                },
            )
...
def get_g4f_providers():
    """Получает список доступных провайдеров из g4f"""
    providers = []
    skip = {'Provider', 'BaseProvider', 'RetryProvider', 'RetryUntilSuccessful',
            'CustomProvider', 'OpenaiChat', 'FlowAI', 'Vercel', 'Yqcloud'}
    ...
    if not providers:
        providers = [
            "PollinationsAI", "Perplexity", "PollinationsAI",
            "DeepInfra", "Together", "Qwen", "Grok", "Copilot"
        ]
...
def get_g4f_models():
    """Получает список доступных моделей из g4f"""
    models = []
    ...
    if not models:
        models = [
            "gpt-4o", "gpt-4o-mini", "gpt-4", "llama-3-70b", "llama-3.1-70b",
            "qwen-2.5-72b", "deepseek-v3", "deepseek-r1", "gemini-2.0-flash",
            "mistral-7b", "mixtral-8x7b", "sonar", "sonar-pro"
        ]
...
def save_test_results(results, total_combinations, elapsed_total, test_mode_name="unknown"):
    """Сохраняет результаты теста в JSON и TXT файлы (с защитой для Railway)"""
    ...
    history = history[-10:]  # Храним только последние 10 тестов

    # 🔧 Записываем безопасно
...
def get_default_candidates() -> List[Tuple[str, str]]:
    """Резервный список комбинаций на основе успешных тестов."""
    return [
...
def get_best_combinations_from_history(max_combinations: int = 5) -> List[Tuple[str, str]]:
    """
    Читает последние 3 теста из test_history.json и возвращает лучшие комбинации.
    """
    history_file = 'test_history.json'
    history = safe_read_json(history_file, [])
    ...
    for (provider, model), data in combination_scores.items():
        ...
        scored_combinations.append({
            ...
            'avg_time': avg_time,
            'avg_rank': avg_rank,
            'appearances': len(data['times']),
            'score': avg_time - stability_bonus
        })
...
def generate_test_report(results, total_tests):
    """Генерирует полный отчёт со ВСЕМИ успешными комбинациями (текстовый)"""
    successful = [r for r in results if r['success']]
...
def generate_test_report_embed(results, total_tests, elapsed_total, test_mode_name):
    """Генерирует ОДИН красивый отчёт в Discord Embed."""
    successful = [r for r in results if r['success']]
    failed = [r for r in results if not r['success']]
    ...
    embed = disnake.Embed(
        title="📊 Отчёт о тестировании моделей",
        description=f"**Режим:** `{test_mode_name}`\n**Время:** `{elapsed_total:.0f}с` ({elapsed_total / 60:.1f} мин)",
        color=color,
    ...
    if successful:
        ...
        embed.add_field(
            name="🏆 Топ-3 быстрых",
            value=top_text if top_text else "❌ Нет данных",
            inline=True
        )
    else:
        embed.add_field(
    ...
    if successful:
        ...
        embed.add_field(
            name="✅ Рабочие (Топ-10)",
            value=f"```\n{table_content}```" if len(
                table_content) < 1024 else "⚠️ Смотрите файл `test_full_report.txt`",
            inline=False
        )
    ...
    if failed:
        ...
        embed.add_field(
            name="❌ Ошибки",
            value=error_text if error_text else "❌ Нет данных",
            inline=False
        )
    ...
    if len(successful) == 0:
        ...
        recommendations = (
            "🟡 **Мало рабочих провайдеров**\n"
            "• Запустите `псинка тест full` 🔍\n"
...
# ============================================================================
# CLASS VIEW С КНОПКАМИ (без изменений)
# ============================================================================

class TestModeView(disnake.ui.View):
    """View с кнопками для выбора режима тестирования"""
    ...
    @disnake.ui.button(label="📊 Полный", style=disnake.ButtonStyle.primary, emoji="📋", custom_id="test_full")
    async def full_button(self, button: disnake.ui.Button, interaction: disnake.MessageInteraction):
        self.selected_mode = "full"
        await interaction.response.defer()
        await self.start_test("full")
    ...
    async def start_test(self, mode: str):
        """Запускает тестирование с выбранным режимом"""
    ...
    async def on_timeout(self):
        """По истечении времени отключаем кнопки"""
        for child in self.children:
            child.disabled = True
        try:
            await self.ctx.message.edit(view=self)
...
# ============================================================================
# ФУНКЦИЯ ЗАПУСКА ТЕСТА (с адаптацией под Railway)
# ============================================================================

async def run_test_command(ctx, mode: str):
    """Запускает тестирование с указанным режимом"""
    try:
        ...
        if mode == "express":
            ...
            providers_to_test = FULL_TEST_NO_AUTH_PROVIDERS
            all_models = FULL_TEST_NO_AUTH_MODELS
            test_mode_name = "🟡 ПОЛНЫЙ ТЕСТ"
            test_openrouter = False
            MAX_CONCURRENT_REQUESTS = int(os.getenv('MAX_CONCURRENT', 5))  # Ограничиваем для Railway
        ...
        async def update_progress(completed: int, total: int, status: str = "🔄", extra: str = ""):
            ...
            try:
                ...
                content = (
                    f"{status} **{test_mode_name}**\n"
                    f"[{bar}] `{completed}/{total}` ({percentage:.1f}%)\n"
                    f"📦 Готово: {len(results)}/{total_combinations}\n"
                    f"⏳ Прошло: {elapsed:.0f}с | ETA: {eta:.0f}с"
                )
                if extra:
                    content += f"\n{extra}"
        ...
        async def test_single_request(provider: str, model: str) -> dict:
            if provider == "OpenRouter":
                ...
                success, answer_or_error, elapsed = await make_g4f_request(provider, model, test_prompt,
                                                                           REQUEST_TIMEOUT)

            return {
                'success': success,
                'time': elapsed if success else None,
                'answer_len': len(answer_or_error) if success else 0,
                'error': answer_or_error if not success else None
            }
        ...
        async def process_combination(provider: str, model: str):
            ...
            async with combo_lock:
                results.append({
                    ...
                    'time': min_time,
                    'success': success,
                    'response_length': 0 if not success else 100,
        ...
        await update_progress(completed_count, total_requests, "✅",
                              f"⏱ Всего: {elapsed_total:.0f}с ({elapsed_total / 60:.1f} мин)")
        ...
        # 🔧 Сохраняем полный отчёт безопасно
...
# ============================================================================
# СПИСКИ ДЛЯ КОМАНДЫ "тест" (без изменений)
# ============================================================================
...
FULL_TEST_NO_AUTH_PROVIDERS = [
    "PollinationsAI", "DeepInfra", "Together", "Perplexity", "HuggingFace",
    "HuggingSpace", "Qwen", "Replicate", "Groq", "Blackbox",
    "DuckDuckGo", "FreeGPT", "Liaobots", "MyShell", "CodeLinkAva", "AiAsk",
    "AiService", "Berlin", "Chatgpt4Online", "ChatgptDemo", "ChatgptDemoAi",
    "ChatgptFree", "ChatgptLogin", "FakeGpt", "FlowAI", "FreeChatgpt",
    "GPTalk", "GeekGpt", "LlamaAI", "Lockchat", "Vercel", "Wuguokai",
    "Ylokh", "You", "Cohere",
]

FULL_TEST_NO_AUTH_MODELS = [
    "gpt-4o", "gpt-4o-mini", "gpt-4", "gpt-3.5-turbo",
    "llama-3-70b", "llama-3.1-70b", "llama-3.1-405b",
    "llama-3.2-3b", "llama-3.2-11b", "llama-3.3-70b",
    "qwen-2-72b", "qwen-2.5-72b", "qwen-2.5-coder-32b",
    "qwen-2-vl-72b", "qwen-2.5-vl-72b",
    "deepseek-v3", "deepseek-r1", "deepseek-r1-turbo",
    "mistral-7b", "mixtral-8x7b", "mistral-nemo",
...
# ============================================================================
# КОМАНДА "тест" С КНОПКАМИ (без изменений)
# ============================================================================

@bot.command(name="тест")
async def тест(ctx, mode: str = None):
    """
    Тестирование моделей g4f + OpenRouter.
    """
    if mode:
    ...
    embed = disnake.Embed(
        ...
        description=(
            ...
            "📋 **Полный** — все провайдеры g4f без авторизации (~15 мин)\n"
            "  `Максимальное покрытие для поиска лучших моделей`\n\n"
    ...
    embed.add_field(
        name="📊 Что будет протестировано",
        value=(
            "• **Скорость ответа** каждой комбинации\n"
            "• **Стабильность соединения** с провайдерами\n"
            "• **Качество ответов** (проверка на русский язык)\n"
            "• **Работоспособность** моделей для команды `скажи`"
        ),
        inline=False
    )
    ...
    embed.add_field(
        name="💾 Результаты",
        value=(
            "• `test_history.json` — история для команды `скажи`\n"
            "• `test_latest_report.txt` — последний отчёт\n"
            "• `test_full_report.txt` — полный отчёт"
        ),
        inline=False
    )
...
async def лучшие(ctx, test_type: str = "all"):
    """
    Показывает лучшие комбинации из истории тестов.
    """
    try:
        history_file = 'test_history.json'
        ...
        type_keywords = {
            "all": ["🟢", "🟡", "🟣", "🔵", "БЫСТРЫЙ", "ПОЛНЫЙ", "OPENROUTER", "АВТО", "ВСЕГО"],
            "quick": ["🟢", "БЫСТРЫЙ"],
            "full": ["🟡", "ПОЛНЫЙ"],
            "openrouter": ["🟣", "OPENROUTER"],
            "auto": ["🔵", "АВТО", "АВТО-СКАНИРОВАНИЕ"],
        }
            ...
            for combo in test.get('top_combinations', []):
                ...
                if key not in combination_stats:
                    combination_stats[key] = {
                        ...
                        'ranks': [],
                        'test_count': 0,
                        'first_seen': test.get('timestamp', ''),
                        'last_seen': test.get('timestamp', '')
                    }
        ...
        embed = disnake.Embed(
            title="🏆 Лучшие комбинации (из test_history.json)",
            description=(
                f"**Тип тестов:** `{test_type}`\n"
                f"**Проанализировано тестов:** `{len(filtered_tests)}`\n"
                f"**Всего комбинаций:** `{len(combination_stats)}`\n\n"
                f"*Данные усреднены по последним 5 тестам каждого типа*"
            ),
            color=0x00FF88,
            timestamp=datetime.now()
        )
        ...
        for i, r in enumerate(top_results, 1):
            medals = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣", "6️⃣", "7️⃣", "8️⃣", "9️⃣", "🔟", "1️⃣1️⃣", "1️⃣2️⃣", "1️⃣3️⃣",
                      "1️⃣4️⃣", "1️⃣5️⃣"]
            ...
            table_rows += (
                f"{medal} `{provider_short:<16}` `{model_short:<18}` "
                f"`{r['avg_time']:.2f}с` `{r['min_time']:.2f}с` {spread:<5} {stability_icon}\n"
            )

        embed.add_field(
            name="📊 Агрегированные результаты",
        ...
        if top_results:
            ...
            stats_text = (
                f"🥇 **Лучшая:** `{best_combo['provider']}` / `{best_combo['model']}` — `{best_combo['avg_time']:.2f}с`\n"
                f"🎯 **Стабильная:** `{most_stable['provider']}` / `{most_stable['model']}` (разброс: `{most_stable['stability']:.1%}`)\n"
                f"📈 **Частая:** `{most_tested['provider']}` / `{most_tested['model']}` (`{most_tested['test_count']}` тестов)\n"
                f"🏆 **Провайдер:** `{best_provider[0]}` (`{best_provider[1]}` комбинаций в топ-15)"
            )
...
async def скажи(ctx, *, prompt: str = None):
    try:
        ...
        if not final_response:
            ...
            try:
                openrouter_response, openrouter_provider, openrouter_model = await chat_via_openrouter(
                    prompt,
                    timeout=40.0
                )
...
async def chat_with_model(prompt: str, timeout: float = 40.0, attempt: int = 1):
    """
    Последовательные попытки с 2 параллельными запросами в каждой попытке.
    """
    ...
    logger.info(
        f"🏎 Попытка {attempt}: 2 параллельных запроса (g4f-default + {current_candidate[0]}/{current_candidate[1]})")
    ...
    def is_error_message(text: str) -> bool:
        error_keywords = [
            'limit', 'limited', 'rate limit', 'too many requests',
            'ip 请求', '限流', '限制', 'error', 'errore',
            'try again', 'попробуйте', 'через', 'минут', 'мин',
            'https://', 'http://', 'wechat', '微信', 'chat19.aichatos',
            '60 次', '每小时', 'ip request', 'temporarily'
        ]
        text_lower = text.lower()
        return any(kw in text_lower for kw in error_keywords)

    async def fetch(provider_name: str, model: str):
        """Делает запрос к модели через g4f."""
        try:
            success, answer, elapsed = await make_g4f_request(
                provider_name, model, prompt,
                timeout=timeout,
                system_prompt="Ты помощник по имени Псинка. Отвечай ТОЛЬКО на русском языке, кратко и максимально по делу."
            )

            logger.info(
                f"📡 g4f запрос {provider_name}/{model}: success={success}, elapsed={elapsed:.2f}s, answer_len={len(answer) if answer else 0}")
    ...
    async def fetch_default_g4f():
        """Запрос через стандартный g4f с моделью deepseek-r1"""
        try:
            ...
            messages = [
                {"role": "system",
                 "content": "Ты помощник по имени Псинка. Отвечай ТОЛЬКО на русском языке, кратко и максимально по делу."},
    ...
    tasks = [
        asyncio.create_task(fetch_default_g4f()),
        asyncio.create_task(fetch(current_candidate[0], current_candidate[1]))
    ]
...
async def chat_via_openrouter(prompt: str, timeout: float = 45.0):
    """
    Fallback запрос через OpenRouter API.
    """
    openrouter_token = os.getenv('OPENR_TOKEN')
...
def get_openrouter_models_from_history(max_models: int = 3) -> List[str]:
    """
    Читает test_history.json и возвращает топ-N самых стабильных моделей OpenRouter.
    """
    history_file = 'test_history.json'
    history = safe_read_json(history_file, [])
    ...
    for i, m in enumerate(top_models, 1):
        ...
        logger.info(
            f"  #{i}: {m} — {model_data['avg_time']:.2f}с (стабильность: {model_data['stability']:.1%}, тестов: {model_data['appearances']})")
...
if __name__ == "__main__":
    try:
        ...
        # На Railway это покажется в логах